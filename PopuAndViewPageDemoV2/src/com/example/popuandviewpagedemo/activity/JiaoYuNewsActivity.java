...
public class JiaoYuNewsActivity extends BaseActivity implements
		IReflashListener {
	List<NewsBean> news_list = new ArrayList<NewsBean>();
	List<NewsBean> jiaoyuLists = new ArrayList<NewsBean>();
	List<NewsBean> jiaoyusLists = new ArrayList<NewsBean>();

	NewsAdapter adapter;
	ReFlashListView listview;
	private String jsonJiaoYu;
	private String jsonJiaoYus;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				httpUtl = new HttpUtl();
				if (httpUtl.hasInternet(JiaoYuNewsActivity.this)) {
					jsonJiaoYu = httpUtl.getJson(URL.JiaoYuNewsIp);
					jsonJiaoYus = httpUtl.getJson(URL.JiaoYusNewsIp);
					if (!TextUtils.isEmpty(jsonJiaoYu)
							&& !TextUtils.isEmpty(jsonJiaoYus)) {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				// TODO Auto-generated method stub
				int pos = arg2 - 1;
				Intent inten = new Intent(JiaoYuNewsActivity.this,
						WebActivity.class);
	...
	private void setData() {
		...
		jiaoyusLists = new JsonParse().parser(jsonJiaoYu);
		jiaoyuLists = new JsonParse().parser(jsonJiaoYus);
		List<NewsBean> beans = new ArrayList<NewsBean>();
		beans.addAll(jiaoyuLists);
		beans.addAll(jiaoyusLists);
		int j = 0;
		int n = 0;
		for (int i = 1; i < beans.size(); i++) {
			if (i % 2 == 0) {
				if (jiaoyuLists.size() > j) {
					news_list.add(jiaoyuLists.get(j++));
				} else if (jiaoyusLists.size() > n) {
					news_list.add(jiaoyusLists.get(n++));
				}
			} else {
				if (jiaoyusLists.size() > n) {
					news_list.add(jiaoyusLists.get(n++));
				} else if (jiaoyuLists.size() > j) {
					news_list.add(jiaoyuLists.get(j++));
				}
