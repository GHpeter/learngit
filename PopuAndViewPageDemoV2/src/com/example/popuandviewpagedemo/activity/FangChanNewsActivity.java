...
public class FangChanNewsActivity extends BaseActivity implements
		IReflashListener {
	List<NewsBean> news_list = new ArrayList<NewsBean>();
	List<NewsBean> fangchanLists = new ArrayList<NewsBean>();
	List<NewsBean> fangchansLists = new ArrayList<NewsBean>();

	NewsAdapter adapter;
	ReFlashListView listview;
	private String jsonFangChan;
	private String jsonFangChans;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				httpUtl = new HttpUtl();
				if (httpUtl.hasInternet(FangChanNewsActivity.this)) {
					jsonFangChan = httpUtl.getJson(URL.FangChanNewsIp);
					jsonFangChans = httpUtl.getJson(URL.FangChansNewsIp);
					if (!TextUtils.isEmpty(jsonFangChan)
							&& !TextUtils.isEmpty(jsonFangChans)) {
		...
		listview.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
				...
				// TODO Auto-generated method stub
				int pos = arg2 - 1;
				Intent inten = new Intent(FangChanNewsActivity.this,
						WebActivity.class);
	...
	private void setData() {
		fangchanLists = new JsonParse().parser(jsonFangChan);
		fangchansLists = new JsonParse().parser(jsonFangChans);

		List<NewsBean> beans = new ArrayList<NewsBean>();
		beans.addAll(fangchanLists);
		beans.addAll(fangchansLists);
		int j = 0;
		int n = 0;
		for (int i = 1; i < beans.size(); i++) {
			if (i % 2 == 0) {
				if (fangchanLists.size() > j) {
					news_list.add(fangchanLists.get(j++));
				} else if (fangchansLists.size() > n) {
					news_list.add(fangchansLists.get(n++));
				}
			} else {
				if (fangchansLists.size() > n) {
					news_list.add(fangchansLists.get(n++));
				} else if (fangchanLists.size() > j) {
					news_list.add(fangchanLists.get(j++));
				}
	...
	private void setReflashData() {
		handler.sendEmptyMessage(1);
		for (int i = 0; i < 1; i++) {
			fangchansLists = new JsonParse().parser(jsonFangChan);
			fangchanLists = new JsonParse().parser(jsonFangChans);
			List<NewsBean> beans = new ArrayList<NewsBean>();
			beans.addAll(fangchanLists);
			beans.addAll(fangchansLists);
			int j = 0;
